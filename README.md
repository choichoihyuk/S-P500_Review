...
4. **📌 내 관심종목** (선택): 사용자가 봇 DM에 티커를 보내면 다음 리포트부터 자동 합류
...
봇 DM(Telegram 채팅창)에 티커를 입력하면 다음 실행(다음 날 KST 07:00)부터 **📌 내 관심종목** 섹션으로 리포트에 포함됩니다. S&P 500 밖 종목(`BE`, `PLTR` 등)도 OK.
...
|---|---|
| `BE` | BE를 관심종목에 추가 |
| `NVDA MSFT AAPL` | 공백/쉼표/개행으로 여러 개 한 번에 |
| `-BE` | BE를 제거 |
| `/list` | 현재 관심종목 조회 |
| `/clear` | 전부 초기화 |

커맨드 수신 직후 다음 cron(KST 07:00)에서 ack 메시지(`✅ 추가: BE` / `📌 현재 관심종목`)를 받습니다. 그리고 같은 run의 리포트 끝에 **📌 내 관심종목** 섹션이 붙습니다.

### 데이터 흐름

- 상태는 `data/watchlist.json`에 저장되고 워크플로가 변경분을 자동 커밋·푸시 (`contents: write` 권한 사용).
- `last_update_id`로 Telegram `getUpdates` offset을 관리 — 한 번 읽은 메시지는 재수신되지 않음.
- 권한 설정: 등록된 `TELEGRAM_CHANNEL_ID`와 일치하는 chat에서 온 메시지만 처리 (봇이 타 유저에게 `/start` 받아도 안전하게 무시).
...
### GH Actions 권한
관심종목 기능 때문에 워크플로가 `contents: write` 권한을 사용합니다 (`data/watchlist.json` 자동 커밋). Repo의 **Settings → Actions → General → Workflow permissions**에서 **"Read and write permissions"** 가 허용되어야 합니다.
